# -*- coding: utf-8 -*-
"""
Created on Tue Sep 29 02:48:14 2020

@author: stefa
"""


def subtitles_data_preparation(csv): 
    subtitles = []
    take = 0
    for l in range(len(csv)-1):
        if take == 1:
            phrase = csv[l][:-2];
...
fi = open("Subtitles/Psycho-Pass_001_script.txt",'r', encoding="utf8")
csv_liste = fi.readlines()
fi.close()

subtitles = subtitles_data_preparation(csv_liste)
...
kanji_autres, kana_autres, nom_trad = data_preparation_noms(csv)
...
for text in subtitles: 
    ...
    for nom in inside_autres:
        for i in range(len(kanji_autres)):
            mot_kanji = kanji_autres[i]
            mot_kana = kana_autres[i]
            trad = nom_trad[i]
            if mot_kanji == nom:
    ...
    for nom in inside_pronoms:
        for i in range(len(pronom_kanji)):
    ...
    if ratio >= 0.95:
        selected_phrases.append(text)
    print(ratio)
        
print(selected_phrases)

recognized_phrases_len = len(selected_phrases) / len(subtitles)
print(recognized_phrases_len)

print('nombre phrase avec grammaire')
print(num_gram)
